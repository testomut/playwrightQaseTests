...
import { MessagesAmount } from '../types/Types';
...
class ClientsPage extends BasePage {
    ...
    async payUponSignup(): Promise<void> {
        const payUponSignupButton = await this.page
            .locator(this.paymentMethodListSelector)
            .filter({ hasText: 'Pay Upon Signup' });
    ...
    async verifyPendingOrganizationList(email: string): Promise<void> {
        ...
        const lastInvitationEmail = await this.page
            .locator(this.pendingOrganizationInvitationsEmailListSelector)
            .first()
            .textContent();
