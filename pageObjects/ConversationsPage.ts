...
class ConversationsPage extends BasePage {
    ...
    private conversationNameSelector: string =
        '[aria-label="ConversationHeaderNameContainerHeader"] [aria-label="TypographyTitle"]';
    ...
    private getNewConversationNewMessageButtonSelector: string =
        '[aria-label="GetNewConversationNewMessageButtonSelector"]';
    ...
    // Endpoints
    private conversationsPageEndpoint: string = `${environmentUrl}/conversations`;
    ...
    async verifyInboxesList(inboxesList: string[]): Promise<void> {
        ...
        const currentInboxesList = await Promise.all(
    ...
    async verifyCompliancePageBeforeRegistration(inboxesList: string[]): Promise<void> {
        ...
        await expect(this.page).toHaveURL(this.conversationsPageEndpoint);
    ...
    async verifyLoginSuccess(): Promise<string> {
        ...
        const firstConversationName = (
            await this.page.locator(this.getInboxListSelector).locator(this.getInboxNameSelector).nth(1).textContent()
        )?.trim();
    ...
    async verifyAssigneeList(assigneeList: string[]): Promise<void> {
        ...
        for (let i = 0; i < assigneeList.length; i++) {
            const assignee = (await this.page
                .locator(this.assigneesListSelector)
                .nth(typographyNameListCount - 1 - i)
    ...
    async verifyConversationDetails(details: ConversationDetails, tab = 1): Promise<void> {
        ...
        const currentConversationDetailsInbox = await targetPage
            .locator(this.conversationDetailsInboxSelector)
            .textContent();
        const currentConversationDetailsInboxPhone = await targetPage
            .locator(this.conversationDetailsInboxPhoneSelector)
            .textContent();
        const currentConversationDetailsAssignee = await targetPage
            .locator(this.conversationDetailsAssigneeSelector)
            .textContent();
    ...
    async foundSearchResultByPhone(sectionSelector: Locator, phone: string): Promise<Locator> {
        ...
        for (let i = 0; i < elementsAmount; i++) {
            const searchResultPhone = await sectionSelector
                .locator(this.searchResultListSelector)
                .nth(i)
                .locator(this.searchResultPhoneSelector)
                .textContent();
    ...
    async verifySearchResult(searchResult: GlobalSearchResult): Promise<void> {
        ...
        if (searchResult && searchResult.contacts) {
            ...
            for (let i = 0; i < searchResult.contacts.length; i++) {
                const searchResultSelector = await this.foundSearchResultByPhone(
                    contactsSection,
                    searchResult.contacts[i].phone,
                );
                const searchResultName = await searchResultSelector
                    .locator(this.searchResultNameSelector)
                    .textContent();
                const searchResultEmail = await searchResultSelector
                    .locator(this.searchResultEmailSelector)
                    .textContent();
    ...
    async verifyConversation(conversation: ConversationData): Promise<void> {
        ...
        if (conversation.name) {
            const conversationName = (await (
                await this.page.locator(this.conversationNameSelector).textContent()
            )?.trim()) as string;
        ...
        if (conversation.lastMessage) {
            await this.verifyText(
                this.page.locator(this.messagelistSelector).locator('[aria-label="TypographyBody"]').last(),
    ...
    async waitNewMessage(beforeMessagesAmount: number, amount?: number): Promise<void> {
        ...
        if (amount) {
            throw new Error(
                `Current messages in conversation: ${currentMessagesAmount} but should be: ${beforeMessagesAmount + amount}`,
            );
        } else {
            throw new Error(
                `Current messages in conversation: ${currentMessagesAmount} but should be more than: ${beforeMessagesAmount}`,
            );
        }
    }
    ...
    async verifyMediaFileInConversation(file: 'audio' | 'img' | 'video' | 'doc' | string): Promise<void> {
        ...
        if (file === 'img' || file === 'video' || file === 'audio') {
            ...
            const url = ((await messageList.locator(file).last().getAttribute('src')) as string)
                .split('blob:')
