...
test.describe('Oauth Functionality', () => {
    test(
        qase(9659, '@QATEST-9659 Create Account (Local Number Registration)'),
        async ({ openAuthPage, emailService }) => {
    ...
    test(
        qase(9634, '@QATEST-9634 Create Account (Select Toll-Free Number)'),
        async ({ openAuthPage, emailService }) => {
            const { authPage, conversationsPage } = openAuthPage;
    ...
    test(qase(9628, '@QATEST-9628 Create Account (Email Address Already Exists)'), async ({ openAuthPage, custom }) => {
        ...
        await custom.wrappedTestStep(
            `2. Enter email address that is already taken->Click"Continue" button.`,
            async () => {
    ...
    test(qase(9631, '@QATEST-9631 Create Account (Invalid Password)'), async ({ openAuthPage, custom }) => {
        ...
        await custom.wrappedTestStep(
            `3. Select any integration from the list -> "Next" button->Select any team size->"Next" button->Select the supported country`,
            async () => {
        ...
        await custom.wrappedTestStep(
            `4. Populate all fields correctly and enter password which is shorter than 7 characters.`,
            async () => {
    ...
    test(qase(9661, '@QATEST-9661 Create Account (Empty Expiration Date)'), async ({ openAuthPage, custom }) => {
        ...
        await custom.wrappedTestStep(
            `4. Populate all fields correctly and leave Credit Card date empty. -> Click "Create My Account" button.`,
            async () => {
                newUser.exp_date = '';
                await authPage.completePersonalDetailsAndCreateAccount(newUser);
    ...
    test(
        qase(9658, '@QATEST-9658 Create Account (Insufficient Credits CC - Verification)'),
        async ({ openAuthPage, custom, emailService }) => {
            ...
            await custom.wrappedTestStep(
                `4. Enter Invalid Credit Card number -> Click "Create My Account" button.`,
                async () => {
            ...
            await custom.wrappedTestStep(
                `5. Change the CC to valid one -> Click "Create My Account" button.`,
                async () => {
    ...
    test(
        qase(9637, '@QATEST-9637 Create Account (with Stripe Coupon)'),
        async ({ signUpWithCoupon, custom, emailService }) => {
            const { authPage, conversationsPage, settingsPage, billingPage } = signUpWithCoupon;
            const newUser = NewUser();
            await custom.wrappedTestStep(
                `1. Populate valid email->Click "Continue" button->Select CRM, Company Size, Country to send message`,
                async () => {
                    await authPage.enterEmailAndContinue(newUser.email);
                    await authPage.verifyTitleChooseCrmPage();
                    await authPage.selectCrmOptionAndContinue('None');
            ...
            await custom.wrappedTestStep(
                `2. Populate all fields with valid data->Click "Create My Account" button.`,
                async () => {
                    await authPage.completePersonalDetailsAndCreateAccount(newUser);
            ...
            await custom.wrappedTestStep(`10. Download and Open a Receipt`, async () => {
                const coupon20Month25 = Coupon20Month25();
    ...
    test(
        qase(9668, '@QATEST-9668 Create Account With Blocked Domain (Straight URL)'),
        async ({ openAuthPage, custom }) => {
            const { authPage } = openAuthPage;
            const newUser = NewUser();
            ...
            await custom.wrappedTestStep(
                `2. In the registration page fill email with free domain (qweqweqwe@gmail.com for example) -> Click to "continue" button`,
                async () => {
            ...
            await custom.wrappedTestStep(
                `3. Go to straight URL https://qatest.com/register?anydomain -> fill email with free domain (qweqweqwe@gmail.com for example) -> Click to "continue" button`,
                async () => {
    ...
    test(qase(9665, '@QATEST-9665 Create Account With Blocked Domain'), async ({ openAuthPage, custom }) => {
        const { authPage } = openAuthPage;
        const newUser = NewUser();
        newUser.email = newUser.email.replace('qatest', 'gmail');
        await custom.wrappedTestStep(`1. Click "Sign Up" button.`, async () => {
        ...
        await custom.wrappedTestStep(
            `2. place email with free domain (qweqweqwe@gmail.com for example) -> Click to "continue" button`,
            async () => {
                await authPage.enterEmailAndContinue(newUser.email, false);
    ...
    test(qase(9657, '@QATEST-9657 Sign Up Flow (Back To The Left Step)'), async ({ openAuthPage, custom }) => {
        const { authPage } = openAuthPage;
        const newUser = NewUser();
        await custom.wrappedTestStep(`1. Click "Sign Up" button.`, async () => {
            await authPage.clickSignUp();
    ...
    test(
        qase(9804, '@QATEST-9804 Successfully logs in with correct credentials'),
        async ({ openAuthPage, userDetails }) => {
            const { authPage, conversationsPage } = openAuthPage;
