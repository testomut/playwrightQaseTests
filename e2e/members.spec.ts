...
test.describe('Members', () => {
    test(qase(65, '@QATEST-65 Invite Member (Member)'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            `2. Enter the email address to Email input field and click to "Add members"`,
            async () => {
        ...
        await custom.wrappedTestStep(`3. Click the "Send Invitations" button`, async () => {
            ...
            await membersPage.verifyPendingMemberTable([newMember]);
        });
    });
    ...
    test(qase(802, '@QATEST-802 Accept Invite (New Inbox) (Local Number)'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            `1. Fill in all fields (First Name, Last Name, Phone Number, Password, Confirm Password, Agreed terms) -> Accept Invite`,
            async () => {
