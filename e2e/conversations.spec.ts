...
// eslint-disable-next-line no-irregular-whitespace -- deliberately includes non-breaking/zero-width characters as part of the special-character coverage this fixture exercises.
...
test.describe('Conversations ', () => {
    test(qase(7288, '@QATEST-7288 Tag Member)'), async ({ start }) => {
        ...
        await test.step(`4. Select a member who is in the inbox`, async () => {
            await conversationsPage.selectMemberAsTagInMessageInput(
                `${userDetails.member?.firstName} ${userDetails.member?.lastName}`,
            );
            await conversationsPage.verifyMessageInputLogic(
                `${userDetails.member?.firstName} ${userDetails.member?.lastName}`,
            );
        });
    ...
    test(qase(10115, '@QATEST-10115 Reassign Conversation'), async ({ start }) => {
        ...
        await test.step(`3. Select different member than it already selected`, async () => {
            ...
            await conversationsPage.verifyLastActivityMessage(
                `${userDetails.firstName} ${userDetails.lastName} reassigned the conversation to ${userDetails.member?.firstName} ${userDetails.member?.lastName}`,
            );
        });
    });
    ...
    test(qase(10306, '@QATEST-10306 Receive Message'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            `1. Make incoming message to user's shared inbox from contact, conversation user focused on`,
            async () => {
    ...
    test(
        qase(
            10309,
            `@QATEST-10309 Start Conversation From All Inboxes (Conversation Doesn't Exist In Any Shared Inbox)`,
        ),
        async ({ start, custom }) => {
            ...
            await custom.wrappedTestStep(
                `1. Find the contact from precondition and click to "Send message" button opposite the contact's name`,
                async () => {
    ...
    test(qase(9716, '@QATEST-9716 Send Message (All Type of Data)'), async ({ start, custom }) => {
        ...
        expect(balance).not.toBe(0);
        await custom.wrappedTestStep(
            `1. Select one conversation. Type a message with symbols, merge fields, image, video, file, vcard, emoticon, short link, custom field`,
            async () => {
    ...
    test(qase(9737, '@QATEST-9737 Send Note'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            '1. Select any conversation ob shared inbox from precondition and click to "SMS"button on high left corner on input field',
            async () => {
                await conversationsPage.toggleSmsNotebutton();
                await conversationsPage.verifyConversation({
                    name: oldAccountPhone,
    ...
    test(qase(6918, '@QATEST-6918 Send Message with Some Segments'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            '1. Type a message that contains some segments (e.g. 2) and send the message',
            async () => {
