...
test.describe('Broadcasts', () => {
    test(
        qase(170, '@QATEST-170 Create and Send Broadcast: Send Now as Owner'),
        async ({ start, custom, customFieldForTest, userBalance }) => {
            ...
            await custom.wrappedTestStep(
                `6. Go to "Conversations" page and select the conversation with contact from broadcast`,
                async () => {
                    ...
                    await conversationsPage.verifyMediaFileInConversation(
                        `broadcast test${oldAccountPhone} ${newCustomFieldValue} 👍`,
                    );
