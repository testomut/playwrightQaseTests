...
const { InviteMemberEmail, SignUpEmail } = require('../constants/emailMessages');
const inviteMemberEmail = InviteMemberEmail();
...
test.describe('Clients', () => {
    test(qase(193, '@QATEST-193 Invite Client / Pro Monthly + Trial (Pay Upon Signup)'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            `3. Inter email to input field for invite user and click "Next" button`,
            async () => {
        ...
        await custom.wrappedTestStep(
            `5. Select a plan by slider and Click to "14-day free trial version" and click to "Next" button`,
            async () => {
    ...
    test(
        qase(1028, '@QATEST-1028 Accept Invite / Pro Monthly + Trial (Pay Upon Signup)'),
        async ({ start, custom }) => {
            ...
            await custom.wrappedTestStep(
                `1. Fill in all fields -> Agree to the terms -> Click the Activate Account button`,
                async () => {
            ...
            await custom.wrappedTestStep(
                `2. Select a state -> Area code -> Select any number -> Next -> Select any country -> Next`,
                async () => {
            ...
            await custom.wrappedTestStep(`4. Go to Email and verify to invitation link`, async () => {
                ...
                inviteMemberEmail.subject = inviteMemberEmail.subject
                    ...
                    .replace('@Organization', userDetails.account);
                await emailService.checkEmail(emailInviteToOrganization, inviteMemberEmail);
                signUpEmail.subject = signUpEmail.subject.replace('John', 'Stefan');
                signUpEmail.body = signUpEmail.body.replace('John', 'Stefan');
