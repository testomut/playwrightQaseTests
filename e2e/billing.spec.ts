...
test.describe('Billing', () => {
    ...
    test(
        qase(70, '@QATEST-70 Upgrade Plan (Trial - Banner) -> Without Toll Free Compliance Flow'),
        async ({ newFreeAccountSignIn, custom }) => {
            const { conversationsPage, settingsPage, billingPage, newUser } = newFreeAccountSignIn;
            await custom.wrappedTestStep(
                `1. Observe the banner "Your trial will expire in 14 days. Upgrade now" -> Click "Upgrade now"`,
                async () => {
            ...
            await custom.wrappedTestStep(
                `2. Observe the banner "Your trial will expire in 14 days. Upgrade now" -> Click "Upgrade now"`,
                async () => {
                    await billingPage.upgrade();
    ...
    test(
        qase(1500, '@QATEST-1500 Downgrade Plan (Annual to Annual) Schedule'),
        async ({ annuallyUpdateAccountSignIn, custom }) => {
    ...
    test(qase(1644, '@QATEST-1644 Add Free Slots of Seat'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            `3. Refresh the page -> Observe the last payment and Seats Remaining`,
            async () => {
                await billingPage.reload();
    ...
    test(qase(2098, '@QATEST-2098 Add Free Slots of Numbers'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            `3. Refresh the page -> Observe the last payment and numbers Remaining`,
            async () => {
                await billingPage.reload();
    ...
    test(qase(1954, '@QATEST-1954 Enable Auto Recharge (User Has 1 CC)'), async ({ start, custom }) => {
        ...
        await custom.wrappedTestStep(
            `3. Observe credits amount in “If Balance Falls Below” drop-down list`,
            async () => {
        ...
        await custom.wrappedTestStep(
            `4. Select credits quantity and Auto Recharge payment method -> Click Save`,
            async () => {
